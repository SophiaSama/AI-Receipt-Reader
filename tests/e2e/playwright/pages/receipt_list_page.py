...
class ReceiptListPage(BasePage):
    ...
    def get_receipt_by_merchant(self, merchant_name: str) -> Locator:
        ...
        return self.page.locator(
            f"{self.RECEIPT_ROW}:has-text('{merchant_name}')"
        ).first
    ...
    def get_receipt_merchants(self) -> List[str]:
        ...
        for i in range(count):
            ...
            merchant_elem = (
                receipt_rows.nth(i)
                .locator("h3, .merchant-name, [data-testid='merchant-name']")
                .first
            )
    ...
    def wait_for_receipts_to_load(self, timeout: int = 5000):
        ...
        receipt_or_empty = (
            self.page.locator(self.RECEIPT_ROW)
            .or_(self.page.locator(self.EMPTY_MESSAGE))
            .or_(self.page.get_by_text("No receipts"))
        )
        ...
        try:
            ...
            receipt_container = self.page.locator("main").or_(
                self.page.locator(".receipt-list")
            )
    ...
    def assert_receipt_exists(self, merchant_name: str):
        ...
        receipt = self.get_receipt_by_merchant(merchant_name)
    ...
    def assert_receipt_not_exists(self, merchant_name: str):
        ...
        # Get all elements with the merchant name text
        all_elements = self.page.get_by_text(merchant_name)
        
        # Check if any of the elements are visible and expect them not to be
    ...
    def assert_stats_visible(self):
        ...
        stats_indicator = (
            self.page.locator(self.STATS_SECTION)
            .or_(self.page.get_by_text("Total Spent"))
            .or_(self.page.get_by_text("Expense Overview"))
        )
