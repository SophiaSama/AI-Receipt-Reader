...
### Handle Duplicate Upload Prompt
```python
def handle_duplicate_prompt_if_present(page: Page, is_duplicate: bool):
    prompt = page.get_by_text("Possible duplicate receipt")
    if prompt.is_visible(timeout=2000):
        if is_duplicate:
